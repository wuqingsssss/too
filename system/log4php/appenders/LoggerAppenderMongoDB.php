...
class LoggerAppenderMongoDB extends LoggerAppender {
   ...
   public function setConverters($pieces){
	...
	public function find($data=array()) {
		try {
			if ($this->collection != null) {
				if(!isset($data['query']))$data['query']=array();
				if(!isset($data['sort']))$data['sort']=array();
				if(!isset($data['limit']))$data['limit']=10;
				
				return $this->collection->find($data['query'])->sort($data['sort'])->limit($data['limit']);
			}
		} catch (MongoCursorException $ex) {
			$this->warn(sprintf('Error while writing to mongo collection: %s', $ex->getMessage()));
		}
...
/*
		...
		if ($locationInfo != null) {
			...
			$document['method']   = DIR_DIR.$this->route.'::'. $locationInfo->getMethodName().'.'.$locationInfo->getClassName();
