...
class Logger {
	...
	public function __construct($name) {
		$this->name = $name;
		$this->logtimes=0;
		if(!isset($GLOBALS['_LOGDATA']))$GLOBALS['_LOGDATA']=array();
		$this->data= &$GLOBALS['_LOGDATA'];
	}
	 ...
	 * @return string
	 */
	public function addLogData($data2) {
	...
	public function find($query=array(),$limit=10,$sort=array("_id"=>-1)){
		$tbs=array();
		$appenderName = $this->getName();
		$data['query']=$query;
		$data['sort']=$sort;
		$data['limit']=$limit;
			foreach($this->appenders as $appender) {
				if(method_exists($appender,'find')){
			 $tb=$appender->find($data);
			  foreach($tb as $doc){
			 	 $tbs[]=$doc;
			  }}
			}
