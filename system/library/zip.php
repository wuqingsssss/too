...
class zip
{
     ...
     * @return    int  影响文件数
     */
    function CompileZipFile($filename, $tozipfilename,$correctname='',$ftype='dir')
    {
        ...
        if (@function_exists('gzcompress'))
        {
            if($ftype=='dir')
            {
                ...
                if(!$correctname)$correctname=dirname($filename);
            }
            else if($ftype=='file')
            {
                $filelist[] =  $filename;
            ...
            if(count($filelist)>0)
            {
                foreach($filelist as $filename)
                {
                    if (is_file($filename))
                    {
                        ...
                        $this->add_File($content, trim(str_replace($correctname,'',$filename),DIRECTORY_SEPARATOR));
                    }
                }
                $out = $this->get_file();
    ...
    function ListDirFiles($dirname)
    {
        ...
        if(is_dir($dirname))
        {
            ...
            {
                if (strcmp($file, '.')==0 || strcmp($file, '..')==0 || stripos($file, '.')===0)
                {
                    continue;
                }
